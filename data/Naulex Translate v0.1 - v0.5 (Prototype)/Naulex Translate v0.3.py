...
GRID_COLS_DEF   = 4
# FIX #1: Debounce lebih tinggi agar grid tidak terlalu sering refresh saat drag
RESIZE_DEBOUNCE = 150
...
def refresh_grid():
    ...
    for name in ordered:
        ...
        # Tentukan style berdasarkan apakah ini bahasa yang dipilih
        ...
        # FIX #1: Hanya panggil configure() jika ada yang berubah
...
def on_resize(event):
    # Abaikan event bukan dari root window (mis. dari child widget)
    if event.widget is not root:
        return
    if _resize_after[0]:
        root.after_cancel(_resize_after[0])
    _resize_after[0] = root.after(RESIZE_DEBOUNCE, _check_and_refresh)

def _check_and_refresh():
    """Hanya refresh grid jika jumlah kolom benar-benar berubah."""
    new_cols = get_column_count()
    if new_cols != _last_col_count[0]:
        refresh_grid()
